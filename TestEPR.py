...
#Running this file will output two plots representing the number of EPR pair received by two ground stations in Paris and in Delft
#The satellite data is given in a separate .npz file. See the CreateNPZ.py file to create new npz files corresponding to specific satellites.
...
for i in range(len(times)):
    ...
    if elSatParis[i] > 20 and elSatDelft[i] > 20:
        ...
        for j in range(numrun):
            ...
            protocolA = ReceiveProtocol(Satellite, Qonnector_meas_succ, Qonnector_meas_flip,True, Paris)
            protocolA.start()

            protocolB = ReceiveProtocol(Satellite, Qonnector_meas_succ, Qonnector_meas_flip,True, Delft)
            protocolB.start()
            stat =ns.sim_run(duration=simtime)
            
            #Adding dark counts
            addDarkCounts(Delft.QlientKeys[Satellite.name], pdarkbest, int(i/Qonnector_init_time))
            addDarkCounts(Paris.QlientKeys[Satellite.name], pdarkbest, int(i/Qonnector_init_time))
            
            L = Sifting(Paris.QlientKeys[Satellite.name], Delft.QlientKeys[Satellite.name]) 
            temp = temp + len(L)
            temp2 =  temp2 +len(L)/len(Satellite.QlientKeys[Paris.name])
            
    print(i)
    keysize.append(temp/numrun)
    throughput.append(temp2/numrun)
    time.append(times[i])
...
plt.savefig("testSatEPR.pdf", format = 'pdf')


plt.figure(figsize=(15,9)) 
plt.plot(time,throughput,label="EPR Throughput")

plt.xlabel('time (min) ',size=20)
plt.ylabel('Throughput (EPR received/EPR sent)',size=20)
plt.legend(loc='upper right',prop={'size':20})

plt.savefig("throughputEPR.pdf", format = 'pdf')
